...
class WPCOM_Member {
    function __construct(){
        ...
        $this->member_path = apply_filters('wpcom_member_path', untrailingslashit(WPMX_DIR));
    ...
    function login_form(){
        ...
        <form id="login-form" class="member-form j-member-form" method="post">
            ...
            <div class="member-remember checkbox">
                ...
                <a class="member-form-forgot" href="<?php echo esc_url(wp_lostpassword_url()); ?>"><?php _e('Forgot password?', 'wpcom');?></a>
            </div>
    ...
    function lostpassword_form_reset(){
        if( isset($GLOBALS['reset-error']) && $GLOBALS['reset-error']){ ?>
            <div class="member-form lostpassword-form lostpassword-form-status">
                ...
                <a class="btn btn-primary" href="<?php echo esc_url(wp_lostpassword_url()); ?>"><?php _e('Click here to resend password reset email', 'wpcom');?></a>
            </div>
        <?php }else{
    ...
    function lostpassword_form_finished(){ ?>
        <div class="member-form lostpassword-form lostpassword-form-status">
            ...
            <a class="btn btn-primary" href="<?php echo esc_url(wp_login_url());?>"><?php _e(' Click here to return to the login page', 'wpcom');?></a>
        </div>
    <?php }
    ...
    function action_before_echo(){
        ...
        if ( is_wpcom_member_page( 'account' ) ) {
            ...
            if( $_SERVER['REQUEST_METHOD'] == 'POST' ){ //表单提交
                ...
                if(isset($_GET['_wpnonce']) && wp_verify_nonce( sanitize_text_field($_GET['_wpnonce']), "wpcom_logout" )){
                    wp_logout();
                }else{
            ...
            $redirect_url = isset( $_SERVER['HTTP_REFERER'] ) && $_SERVER['HTTP_REFERER'] ? sanitize_text_field($_SERVER['HTTP_REFERER']) : '';
            ...
            if($subpage === 'reset'){
                ...
                if ( isset( $_GET['key'] ) ) {
                    $value = sprintf( '%s:%s', sanitize_text_field( $_GET['login'] ), sanitize_text_field( $_GET['key'] ) );
    ...
    function user_has_cap( $allcaps, $caps, $args, $user ){
        ...
        if( $user->ID && ( $pagenow=='user-edit.php' || $pagenow=='users.php' || is_wpcom_member_page() ) ) {
            ...
            }else if( class_exists('WPCOM_User_Groups') && isset($options['member_group']) && $options['member_group'] ){
                // 无用户组则分配默认用户组
    ...
    function user_register( $user_id ){
        $options = $GLOBALS['wpmx_options'];
        if( class_exists('WPCOM_User_Groups') && isset($options['member_group']) && $options['member_group'] ){
            // 分配默认用户组
    ...
    private function set_default_role($user_id, $term_id=''){
        global $options;
        if( $term_id || (class_exists('WPCOM_User_Groups') && isset($options['member_group']) && $options['member_group']) ) {
    ...
    function user_data_stats($user, $link=true){
        ...
        if(class_exists('WPCOM_Follow') && isset($options['member_follow']) && $options['member_follow']=='1'){
