        ...
        if ( isset($_GET['type']) && isset($_GET['action']) ) {
            ...
            if ($_GET['action'] == 'login') {
                ...
                if(isset($_GET['redirect_to']) && $_GET['redirect_to']){
                    // 有跳转回前页，保存到session
                    WPCOM_Session::set('redirect_to', sanitize_text_field($_GET['redirect_to']));
                }
                ...
                if( isset($_GET['uuid']) && $_GET['uuid'] ){
                    ...
                    WPCOM_Session::set('_'.$uuid, sanitize_text_field($_GET['code']));
                    if(isset($_GET['redirect_to']) && $_GET['redirect_to']){
                        // 有跳转回前页，保存到session
                        WPCOM_Session::set('_'.$uuid.'-redirect_to', sanitize_text_field($_GET['redirect_to']));
                    }
                    exit;
                }
            ...
            $html .= '<div class="sl-info-notice">
                    ...
                    <form id="sl-form-create" class="sl-info-form" method="post"><div id="sl-info-nonce">
                        ...
                        <div class="sl-input-item">
                            ...
                            <div class="sl-input">
                                <input class="require" type="text" name="username" value="" placeholder="'.(is_wpcom_enable_phone() ? __('Phone number / E-mail / Username', 'wpcom') : __('Username or email address', 'wpcom')).'">
                            </div>
                        </div>
                        <div class="sl-input-item">
                            <label>' . _x('Password', 'label', 'wpcom') . '</label>
                            <div class="sl-input">
                                <input class="require" type="password" name="password" value="" placeholder="' . _x('Password', 'placeholder', 'wpcom') . '">
                            </div>
                        </div>
                    ...
                    <form id="sl-form-bind" class="sl-info-form" method="post">
                        <div id="sl-info2-nonce">' . wp_nonce_field('wpcom_social_login2', 'social_login2_nonce', true, false) . '</div>
